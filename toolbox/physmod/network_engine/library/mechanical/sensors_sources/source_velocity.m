function source_velocity(root, schema)
    % 1 20
    % 2 20
    % 3 20
    % 4 20
    ...
    % 18 20
    % 19 20
    schema.descriptor = 'Ideal Translational Velocity Source';
    % 21 23
    % 22 23
    S = schema.input('S');
    S.description = 'Control signal';
    S.location = {'Bottom';[0.25]};
    S.type = ne_type('real', [1.0 1.0], 'm/s');
    % 27 28
    C = schema.terminal('C');
...
function setup(root, source_vel)
    % 54 56
    % 55 56
    source_vel.branch_across('v', source_vel.R.v, source_vel.C.v);
    source_vel.branch_through('f', source_vel.R.f, source_vel.C.f);
    % 58 59
...
function e = equation(sys, source_vel)
    ...
    % 69 70
    e = minus(rel_velocity, source_vel.v);
end % function
