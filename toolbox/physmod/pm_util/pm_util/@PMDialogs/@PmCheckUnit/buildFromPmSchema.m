function retStatus = buildFromPmSchema(hThis, pmSchema)
    ...
    % 10 13
    % 11 13
    if not(strcmpi(pmSchema.ClassName, 'PMDialogs.PmCheckUnit'))
        error('PmCheckUnit:buildFromPmSchema:InvalidSchema', xlate('Incorrect schema passed to BuildFromSchema.'));
        retStatus = false;
        return
    end
    % 17 19
...
function retVal = l_CheckForParam(schema, paramName)
    ...
    if eq(retVal, false)
        error('PmCheckUnit:buildFromPmSchema:InvalidSchema', sprintf(xlate('Missing parameter: %s.'), paramName));
        retStatus = false;
    end
end
