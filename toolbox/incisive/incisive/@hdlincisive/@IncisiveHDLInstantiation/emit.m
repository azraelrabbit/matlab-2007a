function hdlcode = emit(this, hC)
    ...
    if eq(length(clkmodes), 1.0)
        ...
        if isempty(clkpathsep)
            error(hdlcodererrormsgid('clockportnumbermismatch'), 'Wrong number of clock ports found in Link For Cadence Incisive block %s', horzcat(get(blkobj, 'Path'), '/', get(blkobj, 'Name')));
            % 24 27
            % 25 27
        end
        [cpath, clkname] = this.incisivedehierarchyname(clkpaths(1.0:minus(clkpathsep(1.0), 1.0)));
    end
    % 29 32
    % 30 32
    portpathsep = find(eq(portpaths, ';'));
    if ne(plus(length(portpathsep), 1.0), length(portmodes))
        error(hdlcodererrormsgid('portnumbermismatch'), 'Wrong number of ports found in Link For Cadence Incisive block %s', horzcat(get(blkobj, 'Path'), '/', get(blkobj, 'Name')));
        % 34 37
        % 35 37
    end
    % 37 39
    ...
    if isempty(portpathsep)
        [notused, entityportnames{plus(end, 1.0)}] = this.incisivedehierarchyname(portpaths);
    else
        st = 1.0;
        testpath = cpath;
        for n=1.0:length(portpathsep)
            [path, entityportnames{plus(end, 1.0)}] = this.incisivedehierarchyname(portpaths(st:minus(portpathsep(n), 1.0)));
        ...
        [path, entityportnames{plus(end, 1.0)}] = this.incisivedehierarchyname(portpaths(st:end));
        if not(isempty(path)) && not(isempty(testpath))
            if not(strcmp(path, testpath))
                error(hdlcodererrormsgid('pathmismatch'), 'All HDL ports must be at a single hierarchical level for HDL code generation');
                % 62 64
