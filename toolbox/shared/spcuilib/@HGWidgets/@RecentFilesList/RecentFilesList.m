function h = RecentFilesList(grp, prf)
    ...
    % 53 55
    % 54 55
    h.loadfileObj = HGWidgets.LoadFile;
end % function
function local_SetPrefGroupName(h, grp)
    ...
    % 60 61
    if not(ischar(grp))
        error('Preference group name must be a string');
    end % if
    h.PrefsGroup = grp;
end % function
function local_SetFileListPrefName(h, prf)
    ...
    % 69 70
    if not(ischar(prf))
        error('File list preference name must be a string');
    end % if
